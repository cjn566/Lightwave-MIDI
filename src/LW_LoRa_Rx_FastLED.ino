...
double timeParameter = 0.0;                 // Normalized time parameter for effects
...
bool isDisplayOn = true;                    // Display power state
...
double tenacityRate, entropyRate, chromaRandomness;     // Calculated effect values
 ...
 * Returns the delta that should be added to reach target hue
 */
double calculateHueDistance(double fromHue, double toHue) {
  double delta = toHue - fromHue;
  
  // Only calculate if difference is significant
  if (abs(delta) > (1.0 / 255.0)) {
    // If distance is > 0.5, go the other way around the color wheel
    if (abs(delta) > 0.5) {
      delta = (delta > 0 ? 1 : -1) - delta;
      delta *= -1;
    }
  }
...
void updateVisualParameter(int paramIndex, uint16_t value) {
  switch (paramIndex) {
    case 0: // Tenacity (fade rate)
      tenacityValue = value;
      tenacityRate = TENACITY_MAX - TENACITY_SCALE * uint5ToFloat(tenacityValue);
      break;
    ...
    case 2: // Chroma (color randomness)
      chromaValue = value;
      chromaRandomness = CHROMA_MIN + CHROMA_SCALE * uint5ToFloat(chromaValue);
      break;
...
void loop() {
  ...
  if (hasReceivedData) {
    ...
    if (_radiolib_status == RADIOLIB_ERR_NONE) {
      ...
      while (rxBuffer[bufferIndex] != 0) {
        ...
        #ifdef DEBUG
          Serial.printf("[%i\t%i]\t", receivedKey, receivedValue);
        #endif
  ...
  // Handle LED output based on current mode
  if (!isDisplayOn) {
    // Display is off - force self mode and clear LEDs
    isInSelfMode = true;
    digitalWrite(MUX_SELECT_PIN, isInSelfMode);
    FastLED.clear();
    FastLED.show();
    
  } else if (isInSelfMode) {
...
void renderLEDVisualization() {
  ...
  renderTime -= frameDelayMs;
  timeParameter = fmod((double)currentMillis / colorCycleTime_ms, 1.0);
  ...
  for (int ledIdx = 0; ledIdx < NUM_LEDS; ledIdx++) {
    ...
    if (ledIntensities[ledIdx] > 0) {
      if (ledIntensities[ledIdx] > tenacityRate && ledIntensities[ledIdx] > 0.01) {
    ...
    if (noteIdx >= 0 && noteIdx < NUM_KEYS && noteIntensities[noteIdx] > 0) {
      ...
      if (currentMillis - noteOnTimestamps[noteIdx] >= STUCK_NOTE_TIMEOUT) {
        ...
        if (isNoteFirstPress[noteIdx]) {
          noteActiveHues[noteIdx] = fmod(timeParameter + (float)random(0, (long)(chromaRandomness * 1000)) / 1000, 1.0);
        }
    ...
    if (ledIdx > 0) {
      ...
      // Hue propagation and smoothing logic from C++ version
